...
export default class Canvas extends React.Component {
  ...
  renderSource(value) {
    import('../../src').then(Essence => {
      const args = ['context', 'React', 'ReactDOM'];
      const argv = [this, React, ReactDOM];

      for (const key in Essence) {
        args.push(key);
        argv.push(Essence[key]);
      }
        ...
        <div className="demo-block-control" onClick={this.blockControl.bind(this)}>
          {
            this.state.showBlock ? (
              <span>
                <i className="fa fa-caret-up" />{this.props.locale.hide}
              </span>
            ) : (
              <span>
                <i className="fa fa-caret-down" />{this.props.locale.show}
              </span>
            )
          }
        </div>
