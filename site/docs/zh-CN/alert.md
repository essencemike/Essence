...
render() {
  return (
    <div>
      <Alert title="成功提示的文案" type="success" showIcon={true} banner />
      <Alert title="消息提示的文案" type="info" showIcon={true} banner />
      <Alert title="警告提示的文案" type="warning" showIcon={true} banner />
      <Alert title="错误提示的文案" type="error" showIcon={true} banner />
    </div>
  )
}
```
